...
func main() {
	...
	/*
		...
		в нашей строке подключения к MySQL, чтобы заставить его преобразовывать поля TIME и DATE в time.Time.
		В противном случае, он вернёт их как объекты []byte. Это один из многих предлагаемых параметров
		для предварительной настройки драйвера базы данных при подключении к нему.
	*/
	flag.Parse()
